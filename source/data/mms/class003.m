      ...
      for j=1:ntypes;
        ...
        for ii=1:ntypes;
          ...
          if ~isempty(tem2); 
            ...
            if tem4>1;
              ...
              eval(['sclasst',k,ll,m,'=-2*ones(1,l2+4);'])
            end;
          else;
            eval(['sclasst',k,ll,m,'=-2*ones(2,l2+4);'])
          end;
        end;
        cstr=['CSc',k,'(:,1:l)'];
        tem1=bnames(j,:);
        ...
        if ~isempty(tem2); 
          ...
          if tem4>1;
            ...
            eval(['sclassc',k,ll,'=-2*ones(1,l+3);'])
          end;
        else;
          eval(['sclassc',k,ll,'=-2*ones(2,l+3);'])
        end;
      end;
    end;
  end;
...
%  if ~rem(it,Tgat);
  if runit(it)==1;
    ...
    for i=1:nsend;
      ...
      eval(['[CSt',k,',last(:,',k,')]=ga3(CSt',k,',1,nselectt,pcrosst', ..
...
%  if ~rem(it,Tgac);
  if runit(it)==1;
    ...
    for i=1:nsend;
      k=int2str(send(i));
      fprintf('Genetic Algorithm for Classifier System %g \n',send(i))
      eval(['[CSc',k,',last(:,',k,')]=ga3(CSc',k,',2,nselectc,pcrossc', ..
