...
tem1=max(CS(:,usepos));
cankill=find( CS(:,fitpos)<uratio(1) | CS(:,usepos)/tem1 <uratio(2));
...
disp('Pair    Mate1   Mate2   SiteCross   Mort1   Mort2  Iteration')
disp('------------------------------------------------------------')
for j=1:nselect;
  if ncalled<nclass;
    tem1=CS(:,usepos)+1;       
  ...
  if rand<pcross;             %  find crossover point if crossing to be done
    jcross=1+floor((lcond-1)*rand);
    ncross=ncross+1;
  else;
    jcross=lcond;
  end;
  rnd1=(rand(1,lcond)<pmutation);  %  find bits to be mutated if any
  rnd2=(rand<pmutation);
  v=[CS(mate1,1:jcross),CS(mate2,jcross+1:lcond)];
  av=(CS(mate1,fitpos)+CS(mate2,fitpos))*.5;
  child1=[(1-rnd1).*v+rnd1.*(rem(v+ceil(rand(v)*2)+1,3)-1), ..
          abs(CS(mate1,actpos)-rnd2),av];  %  create first child ...

  rnd3=(rand(1,lcond)<pmutation);
  rnd4=(rand<pmutation);
  v=[CS(mate2,1:jcross),CS(mate1,jcross+1:lcond)];
  child2=[(1-rnd3).*v+rnd3.*(rem(v+ceil(rand(v)*2)+1,3)-1), ..
          abs(CS(mate2,actpos)-rnd4),av];  %  and second child
  ...
  disp([j,mate1,mate2,jcross,mort1,mort2,iteration])
end;
