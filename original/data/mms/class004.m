...
if rclass;
  ...
  for i=1:ntypes;
    k=int2str(i);
    eval(['CSt',k,'=[CSt,zeros(nclasst,4)];'])
    eval(['CSc',k,'=[CSc,zeros(nclassc,3)];'])
  end;
end;

%disp(' WARNING: the strengths of CSt,CSc have been altered!')

%ind1=[2,4,6,8,10,12,13,16,17,21,25,28,30,32,37,40,52,64]';
%ind2=[2,3,6,10,14,16,18,20,22,24,25,27,30,35,42,51,54,62,66,70]';
%ind3=[2,4,5,12,16,17,26,28,30,32,34,36,40,50,52,60,64,65]';
...
%
%  (c) initialize matrices used in storing intermediate information
%      on the histograms, frequencies, classifiers
%
shist=[ ];
for i=1:ntypes;
  k=int2str(i);
...
%
%  (d) initialize the storages for the population
%
popstorage=bnames(1+floor(rand(total,1)*ntypes),:);
%
%  (e) initialize the indices for winning consume classifiers of last 
%      iteration (t-1) who will be rewarded bid payments during the 
%      current iteration (t)
%
last=zeros(max(nagents),ntypes);                
%
%  (f) initialize matrices used in printing results:
%      (i) vectors initgat and initgac used for controlling printing
%          of classifiers during inital genetic algorithm calls
%     (ii) string vectors prtstr and prcstr which are labels that 
%          appear above columns when classifiers are printed
%
initgat=ones(ntypes,1);
initgac=initgat;   
prcstr=[' #  ',setstr(32*ones(1,ceil((l-1)/2))),'CSc',  ..
        setstr(32*ones(1,floor((l-1)/2))), ..
        '  Strength     # used  orig.'];
prtstr=[' #  ',setstr(32*ones(1,ceil((l2-1)/2))),'CSe', ..
        setstr(32*ones(1,floor((l2-1)/2))),  ..
        '  Strength   # traded    # used  orig.'];
for i=1:max(nclasst,nclassc);
  block1(i,:)=[sprintf('%4g',i),' '];
end;
%
%  (g) initialize the weights used when updating strengths
%
Pt=zeros(nclasst,ntypes);
Pc=zeros(nclassc,ntypes);
%
%  (h) and print out parameters and original classifier systems.
%
disp(' ')
...
fprintf(' Classifier Systems randomly generated (1) or input (0): %g\n',rclass)
...
string=' Number of periods between storing information from classifiers = %g\n';
fprintf(string,savec)
string=' Number of periods between displays of histograms = %g\n';
fprintf(string,dhist)
string=' Number of periods between storing histograms = %g\n';
fprintf(string,saveh)
string=' Number of periods between displays of probability matrices = %g\n';
fprintf(string,dprob)
string=' Number of periods between storing frequency information = %g\n';
fprintf(string,savef)
fprintf(' Number of periods used for computing statistics = %g\n',nback)
disp(' Iterations with genetic algorithm call given in RUNIT')
string=[' Probability that a second classifier system goes to gen. alg. ', ..
         'given one sent = %g\n'];
fprintf(string,psecond)
string=[' Probability that a third classifier system goes to gen. alg. ', ..
         'given two sent = %g\n'];
fprintf(string,pthird)
...
string=[' Proportion of most used rules in trade classifier chosen as ', ..
         'candidates in reproduction = %g\n'];
fprintf(string,propmostusedt)
string=[' Proportion of most used rules in consume classifier chosen as ', ..
         'candidates in reproduction = %g\n'];
fprintf(string,propmostusedc)
fprintf(' Criteria for rule elimination in genetic algorithm: %g, %g', ..
          uratio(1),uratio(2))
...
for i=1:ntypes;
  ...
  disp(' ')
  for j=1:nclasst;
    eval(['tem1=CSt',k,'(j,l2+2:l2+5);'])
    block2(j,:)=[sprintf('%12.4e%10g',tem1(1),tem1(2)),  ..
                 sprintf('%10g%6g',tem1(3),tem1(4))];
  end;
  eval(['tem1=CSt',k,'(:,1:l2+1);'])
  tem1=49*(tem1>0)+35*(tem1<0)+48*(tem1==0);
  disp([prtstr;block1(1:nclasst,:),tem1,block2])
  disp(' ')
  fprintf('  Consume Classifier System for Type %g Agents: \n',i)
  disp('  ----------------------------------------------')
  disp(' ')
  for j=1:nclassc;
    eval(['tem1=CSc',k,'(j,l+2:l+4);'])
    block3(j,:)=sprintf('%12.4e%10g%6g',tem1(1),tem1(2),tem1(3));
  end;
  eval(['tem1=CSc',k,'(:,1:l+1);'])
  tem1=49*(tem1>0)+35*(tem1<0)+48*(tem1==0);
  disp([prcstr;block1(1:nclassc,:),tem1,block3])
  disp(' ')
end;
%
%
...
for it=1:maxit
  ...
  for i=1:halftot;
    %
    % (a) compute the weights used in updating strengths:
    %
    for j=1:ntypes;
      k=int2str(j);
      eval(['tem1=CSt',k,'(:,l2+1);'])
      eval(['Pt(:,j)=tem1.*CSt',k,'(:,l2+3)+(1-tem1).*CSt',k,'(:,l2+4)+1;'])
      eval(['Pc(:,j)=CSc',k,'(:,l+3)+1;'])
    end;
    %
    % (b) get conditions: [own storage, match's storage],
    %
    ...
    %
    % (c) get strings type1 and type2 giving agent types,
    %
    type1=int2str(mate1(i,2));
    type2=int2str(mate2(i,2));
    %
    % (d) update freq3<i>(j,k) and freq2<i>(j)
    %
    %
    ...
    eval([fstr,'=',fstr,'+1;'])
    %
    % (e) find indices of classifiers in CSt matching conditions and
    ...
    %
    % (f) find matching classifiers with winning bids ..
    %
    eval(['c1=[ind3,CSt',type1,'(ind3,[l2+2,l2+1,l2+4])];'])
    eval(['c2=[ind4,CSt',type2,'(ind4,[l2+2,l2+1,l2+4])];'])
    win1=find(c1(:,2)>=max(c1(:,2)));
    win2=find(c2(:,2)>=max(c2(:,2)));
    win1=c1(win1(1+floor(rand*length(win1))),:);
    win2=c2(win2(1+floor(rand*length(win2))),:);
    if ~any(c1(:,3)-win1(3));
      eval(['lose1=find(CSt',type1,'(:,l2+4)./(1+max(CSt',type1, ..
            '(:,l2+4)))<ufitness);'])
      if ~isempty(lose1);
        eval(['tem1=find(CSt',type1,'(lose1,l2+2)<=min(CSt',type1, ..
              '(lose1,l2+2)));'])
        eval(['CSt',type1,'(lose1(tem1(1)),:)=CSt',type1,'(win1(1),:);'])
        eval(['CSt',type1,'(lose1(tem1(1)),[l2+1,l2+5])=[~win1(3),it];'])
      end;
    end;
    if ~any(c2(:,3)-win2(3));
      eval(['lose2=find(CSt',type2,'(:,l2+4)./(1+max(CSt',type2, ..
            '(:,l2+4)))<ufitness);'])
      if ~isempty(lose2);
        eval(['tem1=find(CSt',type2,'(lose2,l2+2)<=min(CSt',type2, ..
              '(lose2,l2+2)));'])
        eval(['CSt',type2,'(lose2(tem1(1)),:)=CSt',type2,'(win2(1),:);'])
        eval(['CSt',type2,'(lose2(tem1(1)),[l2+1,l2+5])=[~win2(3),it];'])
      end;
    end;
    %
    % (g) and their strings,
    ...
    %
    % (h) and count the number of #'s  (here represented as -1)
    %
    ...
    eval(['tem1=(CSt',type1,'(win1(1),l2+4)./(1+max(CSt',type1,  ..
                            '(ind3,l2+4)))>ufitness);'])
    if sp1 & tem1 & runit1(it);
      eval(['rnd=(rand(1:l2+1)<pmutationt & string1<zeros(1,l2+1));'])
      if sum(rnd); 
        eval(['lose1=find(CSt',type1,'(:,l2+4)./(1+max(CSt',type1, ..
              '(:,l2+4)))<ufitness);'])
        if ~isempty(lose1);
          eval(['tem1=find(CSt',type1,'(lose1,l2+2)<=min(CSt',type1, ..
                '(lose1,l2+2)));'])
          eval(['CSt',type1,'(lose1(tem1(1)),:)=[CSt',type1,  ..
                '(win1(1),1:l2+1).*(~rnd)+(rand(1,l2+1)<.5).*rnd,', ..
                'CSt',type1,'(win1(1),l2+2:l2+4),it];'])
        end;
      end;
    end;
    sp1=1/(1+sp1);
    ...
    eval(['tem1=(CSt',type2,'(win2(1),l2+4)./(1+max(CSt',type2,  ..
                            '(ind4,l2+4)))>ufitness);'])     
    if sp2 & tem1 & runit1(it);
      eval(['rnd=(rand(1:l2+1)<pmutationt & string2<zeros(1,l2+1));'])
      if sum(rnd); 
        eval(['lose2=find(CSt',type2,'(:,l2+4)./(1+max(CSt',type2, ..
              '(:,l2+4)))<ufitness);'])
        if ~isempty(lose2);
          eval(['tem1=find(CSt',type2,'(lose2,l2+2)<=min(CSt',type2, ..
                '(lose2,l2+2)));'])
          eval(['CSt',type2,'(lose2(tem1(1)),:)=[CSt',type2,  ..
                '(win2(1),1:l2+1).*(~rnd)+(rand(1,l2+1)<.5).*rnd,', ..
                'CSt',type2,'(win2(1),l2+2:l2+4),it];'])
        end;
      end;
    end;
    sp2=1/(1+sp2);
    % 
    % (i) determine if agents traded:
    % 
    eval(['trade1=(CSt',type1,'(win1(1),l2+1) );'])
    eval(['trade2=(CSt',type2,'(win2(1),l2+1) );'])
    trade=(trade1 & trade2);
    %
    % (j) get conditions: own storage     ... for CSc matrices
    %
    if trade;
    ...
    %
    % (k) reward bids to sources of current winners 
    %
    ...
    if source>0;
      if ( (trade1==trade2) | trade1==0 );
        ...
        eval(['tem1=Pc(source,',type1,');'])
        tem2=1/tem1;
        tem1=1-tem2;
        eval([rstr,'=tem1*',rstr,'+tem2*cbid*CSt',type1,'(win1(1),l2+2);'])
      end;
    end;
    if ( (trade1==trade2) | trade1==0 );
      eval(['tem1=Pt(win1(1),',type1,');'])
      tem2=1/tem1;
      tem1=1-tem2;
      eval(['CSt',type1,'(win1(1),l2+2)=CSt',type1, ..
            '(win1(1),l2+2)*(tem1-tem2*cbid);'])
    end;

    cbid=tbid1(mate2(i,2))+tbid2(mate2(i,2))*sp2;
    source=last(mate2(i,1),mate2(i,2));
    if source>0;
      if ( (trade1==trade2) | trade2==0 );
        rstr=['CSc',type2,'(source,l+2)'];
        eval(['tem1=Pc(source,',type2,');'])
        tem2=1/tem1;
        tem1=1-tem2;
        eval([rstr,'=tem1*',rstr,'+tem2*cbid*CSt',type2,'(win2(1),l2+2);'])
      end;
    end;
    if ( (trade1==trade2) | trade2==0 );
      eval(['tem1=Pt(win2(1),',type2,');'])
      tem2=1/tem1;
      tem1=1-tem2;
      eval(['CSt',type2,'(win2(1),l2+2)=CSt',type2, ..
            '(win2(1),l2+2)*(tem1-tem2*cbid);'])
    end;
    %
    % (l) find indices of classifiers in CSc matching conditions and
    %     if there are no matches, replace a string with the condition,
    %
    cstr=['CSc',type1,'(:,1:l)'];
    ...
    %
    % (m) find matching classifiers with winning bids ..
    %
    eval(['c1=[ind5,CSc',type1,'(ind5,[l+2,l+1])];'])
    eval(['c2=[ind6,CSc',type2,'(ind6,[l+2,l+1])];'])
    win3=find(c1(:,2)>=max(c1(:,2)));
    win4=find(c2(:,2)>=max(c2(:,2)));
    win3=c1(win3(1+floor(rand*length(win3))),:);
    win4=c2(win4(1+floor(rand*length(win4))),:);
    if ~any(c1(:,3)-win3(3));
      eval(['lose1=find(CSc',type1,'(:,l+3)./(1+max(CSc',type1, ..
            '(:,l+3)))<ufitness);'])
      if ~isempty(lose1);
        eval(['tem1=find(CSc',type1,'(lose1,l+2)<=min(CSc',type1, ..
              '(lose1,l+2)));'])
        eval(['CSc',type1,'(lose1(tem1(1)),:)=CSc',type1,'(win3(1),:);'])
        eval(['CSc',type1,'(lose1(tem1(1)),[l+1,l+4])=[~win3(3),it];'])
      end;
    end;
    if ~any(c2(:,3)-win4(3));
      eval(['lose2=find(CSc',type2,'(:,l+3)./(1+max(CSc',type2, ..
            '(:,l+3)))<ufitness);'])
      if ~isempty(lose2);
        eval(['tem1=find(CSc',type2,'(lose2,l+2)<=min(CSc',type2, ..
              '(lose2,l+2)));'])
        eval(['CSc',type2,'(lose2(tem1(1)),:)=CSc',type2,'(win4(1),:);'])
        eval(['CSc',type2,'(lose2(tem1(1)),[l+1,l+4])=[~win4(3),it];'])
      end;
    end;
    %
    % (n) and their strings,
    ...
    %
    % (o) and count the number of #'s  (here represented as -1)
    %
    sp1=sum(string1(1:l)<0)*isempty(find(~sum(abs(bnames-ones(ntypes,1)* ..
        string1(1:l))')));
    eval(['tem1=(CSc',type1,'(win3(1),l+3)./(1+max(CSc',type1,  ..
                            '(ind5,l+3)))>ufitness);'])     
    if sp1 & tem1 & runit1(it);
      eval(['rnd=(rand(1:l+1)<pmutationc & string1<zeros(1,l+1));'])
      if sum(rnd); 
        eval(['lose1=find(CSc',type1,'(:,l+3)./(1+max(CSc',type1, ..
              '(:,l+3)))<ufitness);'])
        if ~isempty(lose1);
          eval(['tem1=find(CSc',type1,'(lose1,l+2)<=min(CSc',type1, ..
                '(lose1,l+2)));'])
          eval(['CSc',type1,'(lose1(tem1(1)),:)=[CSc',type1,  ..
                '(win3(1),1:l+1).*(~rnd)+(rand(1,l+1)<.5).*rnd,', ..
                'CSc',type1,'(win3(1),l+2:l+3),it];'])
        end;
      end;
    end;
    sp1=1/(1+sp1);
    sp2=sum(string2(1:l)<0)*isempty(find(~sum(abs(bnames-ones(ntypes,1)* ..
        string2(1:l))')));
    eval(['tem1=(CSc',type2,'(win4(1),l+3)./(1+max(CSc',type2,  ..
                            '(ind6,l+3)))>ufitness);'])     
    if sp2 & tem1 & runit1(it);
      eval(['rnd=(rand(1:l+1)<pmutationc & string2<zeros(1,l+1));'])
      if sum(rnd); 
        eval(['lose2=find(CSc',type2,'(:,l+3)./(1+max(CSc',type2, ..
              '(:,l+3)))<ufitness);'])
        if ~isempty(lose2);
          eval(['tem1=find(CSc',type2,'(lose2,l+2)<=min(CSc',type2, ..
                '(lose2,l+2)));'])
          eval(['CSc',type2,'(lose2(tem1(1)),:)=[CSc',type2,  ..
                '(win4(1),1:l+1).*(~rnd)+(rand(1,l+1)<.5).*rnd,', ..
                'CSc',type2,'(win4(1),l+2:l+3),it];'])
        end;
      end;
    end;
    sp2=1/(1+sp2);
    % 
    % (p) determine if agents consume:
    % 
    eval(['cons1=CSc',type1,'(win3(1),l+1);'])
    eval(['cons2=CSc',type2,'(win4(1),l+1);'])
    %
    % (q) reward bids to sources of current winners 
    %
    ...
    if ( (trade1==trade2) | trade1==0 );
      ...
      tem2=1/tem1;
      tem1=1-tem2;
      eval([rstr,'=tem1*',rstr,'+tem2*cbid*CSc',type1,'(win3(1),l+2);'])
    end;
    eval(['tem1=Pc(win3(1),',type1,');'])
    tem2=1/tem1;
    tem1=1-tem2;
    tem3=cons1*(( ind7==mate1(i,2) )*utility(mate1(i,2))-  ..
         prodcosts(mate1(i,2)) )-storecosts(mate1(i,2),ind7);
    eval(['CSc',type1,'(win3(1),l+2)=CSc',type1, ..
          '(win3(1),l+2)*(tem1-tem2*cbid)+tem2*tem3;'])
    last(mate1(i,1),mate1(i,2))=win3(1);

    cbid=cbid1(mate2(i,2))+cbid2(mate2(i,2))*sp2;
    if ( (trade1==trade2) | trade2==0 );
      rstr=['CSt',type2,'(win2(1),l2+2)'];
      eval(['tem1=Pt(win2(1),',type2,');'])
      tem2=1/tem1;
      tem1=1-tem2;
      eval([rstr,'=tem1*',rstr,'+tem2*cbid*CSc',type2,'(win4(1),l+2);'])
    end;
    eval(['tem1=Pc(win4(1),',type2,');'])
    tem2=1/tem1;
    tem1=1-tem2;
    tem3=cons2*(( ind8==mate2(i,2) )*utility(mate2(i,2))-  ..
         prodcosts(mate2(i,2)) )-storecosts(mate2(i,2),ind8);
    eval(['CSc',type2,'(win4(1),l+2)=CSc',type2, ..
          '(win4(1),l+2)*(tem1-tem2*cbid)+tem2*tem3;'])
    last(mate2(i,1),mate2(i,2))=win4(1);
    %
    % (r) determine next period's storage
    %
    if cons1;
    ...
    %
    % (s) update the number of times the rule was called and the number
    %      of exchanges,
    %
    ...
    %
    % (t) update the freq1<i>, freq4<i>, and freq5<i> matrices
    %
    ...
    eval([fstr,'=',fstr,'+trade;'])
    eval(['freq5',type1,'(ind7,1)=freq5',type1,'(ind7,1)+cons1;'])
    eval(['freq5',type2,'(ind8,1)=freq5',type2,'(ind8,1)+cons2;'])
    fstr=['freq5st',type1,'(nback,ind7)'];
    eval([fstr,'=',fstr,'+cons1;'])
    fstr=['freq5st',type2,'(nback,ind8)'];
  ...
  for i=1:ntypes;
    k=int2str(i);
    eval(['CSt',k,'(:,l2+2)=CSt',k,'(:,l2+2)-ones(nclasst,1)./Pt(:,',  ..
           k,').*(CSt',k,'(:,l2+2)+Taxt(:,i));'])
    eval(['CSc',k,'(:,l+2)=CSc',k,'(:,l+2)-ones(nclassc,1)./Pc(:,',  ..
           k,').*(CSc',k,'(:,l+2)+Taxc(:,i));'])
  ...
  %
  %  Store frequency, classifier, and histogram information if requested 
  %
  if ~rem(it,savef);
    for i=1:ntypes;
      k=int2str(i);
      ...
      for j=1:ntypes;
        ...
        for ii=1:ntypes;
          ...
          if ~isempty(tem2); 
            ...
            tem3=tem2(tem3);
            tem4=length(tem3);
            eval(['sclasst',k,ll,m,'=[sclasst',k,ll,m,';CSt',k,  ..
                  '(tem3(tem4),1:l2+4)];'])
            if tem4>1;
              eval(['sclasst',k,ll,m,'=[sclasst',k,ll,m,';CSt',k,  ..
                    '(tem3(tem4-1),1:l2+4)];'])
            else;
              eval(['sclasst',k,ll,m,'=[sclasst',k,ll,m,';-2*ones(1,l2+4)];'])
            end;
          else;
            eval(['sclasst',k,ll,m,'=[sclasst',k,ll,m,';-2*ones(2,l2+4)];'])
          end;
        end;
        cstr=['CSc',k,'(:,1:l)'];
        tem1=bnames(j,:);
        ...
        if ~isempty(tem2); 
          ...
          tem3=tem2(tem3);
          tem4=length(tem3);
          eval(['sclassc',k,ll,'=[sclassc',k,ll,';CSc',k,  ..
                '(tem3(tem4),1:l+3)];'])
          if tem4>1;
            eval(['sclassc',k,ll,'=[sclassc',k,ll,';CSc',k,  ..
                  '(tem3(tem4-1),1:l+3)];'])
          else;
            eval(['sclassc',k,ll,'=[sclassc',k,ll,';-2*ones(1,l+3)];'])
          end;
        else;
          eval(['sclassc',k,ll,'=[sclassc',k,ll,';-2*ones(2,l+3)];'])
        end;
      end;
    end;
  end;
...
%                k,'./(freq2s',k,'*ones(1,ntypes))]) '])
  ...
  %
  %  If RUNIT(it) is on, run the genetic algorithm for CSt.
  %
  if runit2(it)==1;
    ...
    if ~rem(it,dclasst);
      for i=1:ntypes;
        k=int2str(i);
        disp(' ')
        s=['  Trade Classifier System for Type %g Agents before Gen. ', ..
           'Alg., iteration %g: \n'];
        fprintf(s,i,it)
        disp(['  -------------------------------------------------------', ..
           '------------------'])
    ...
    for i=1:nsend;
      ...
      eval(['[CSt',k,',last(:,',k,')]=ga4(CSt',k,',1,nselectt,pcrosst', ..
            ',pmutationt,crowdfactort,crowdsubpopt,nclasst,l2', ..
            ',last(:,',k,'),it,propmostusedt,uratio);'])
    end;
    if ~rem(it,dclasst);  
      for i=1:ntypes;
        k=int2str(i);
        disp(' ')
        s=['  Trade Classifier System for Type %g Agents after Gen. ', ..
           'Alg., iteration %g: \n'];
        fprintf(s,i,it)
        disp(['  -------------------------------------------------------', ..
           '---------------------'])
  ...
  %
  %  If RUNIT(it) is on, run the genetic algorithm for CSc.
  %
  if runit2(it)==1;
    send=1+floor(rand*ntypes);
    if psecond>rand; 
    ...
    if ~rem(it,dclassc);
      for i=1:ntypes;
        k=int2str(i);
        disp(' ')
        s=['  Consume Classifier System for Type %g Agents before ', ..
           'Gen. Alg., iteration %g: \n'];
        fprintf(s,i,it)
        disp(['  -------------------------------------------------------', ..
           '---------------------'])
    ...
    for i=1:nsend;
      k=int2str(send(i));
      fprintf('Genetic Algorithm for Classifier System %g \n',send(i))
      eval(['[CSc',k,',last(:,',k,')]=ga4(CSc',k,',2,nselectc,pcrossc', ..
            ',pmutationc,crowdfactorc,crowdsubpopc,nclassc,l', ..
            ',last(:,',k,'),it,propmostusedc,uratio);'])
    end;
    if ~rem(it,dclassc);  
      for i=1:ntypes;  
        k=int2str(i);
        disp(' ')
        s=['  Consume Classifier System for Type %g Agents after ', ..
           'Gen. Alg., iteration %g: \n'];
        fprintf(s,i,it)
        disp(['  -------------------------------------------------------', ..
           '---------------------'])
  ...
  if ~rem(it,dclasst) & runit2(it)==0; 
    for i=1:ntypes;
      k=int2str(i);
      disp(' ')
      s='  Trade Classifier System for Type %g Agents at iteration %g: \n';
      fprintf(s,i,it)
      disp('  -----------------------------------------------------------')
      disp(' ')
      for j=1:nclasst;
  ...
  if ~rem(it,dclassc) & runit2(it)==0;
    for i=1:ntypes;
      k=int2str(i);
      disp(' ')
      s='  Consume Classifier System for Type %g Agents at iteration %g: \n';
      fprintf(s,i,it)
      disp('  -------------------------------------------------------------')
      disp(' ')
      for j=1:nclassc;
